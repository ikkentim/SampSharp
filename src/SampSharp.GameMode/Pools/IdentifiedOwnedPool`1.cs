...
/// <summary>Keeps track of a pool of owned and identified instances.</summary>
...
public abstract class IdentifiedOwnedPool<TInstance, TOwner> : Disposable, IIdentifiable, IOwnable<TOwner>
    where TInstance : IdentifiedOwnedPool<TInstance, TOwner> where TOwner : class, IIdentifiable
{
    ...
    /// <summary>Initializes a new instance of the <see cref="IdentifiedOwnedPool{TInstance, TOwner}" /> class.</summary>
    ...
    public static IEnumerable<TInstance> All => _containers.SelectMany(c => c.Value)
        .Concat(_unownedContainer)
        .ToArray();
    ...
    /// <summary>Gets the owner of this instance.</summary>
    public TOwner Owner
    {
        get => _owner;
        protected set
        {
            ...
            else
            {
                if (_id == PoolContainer<TInstance>.UnidentifiedId)
                    _containers[_owner]
                        .RemoveUnidentified((TInstance)this);
                else
                    _containers[_owner]
                        .Remove(_id);

                if (!_containers[_owner]
                        .Any())
                    _containers.Remove(_owner);
            }

            GetPool(value)
                .Add(_id, (TInstance)this);
    ...
    /// <summary>Gets a collection of instanced owned by the specified owner.</summary>
    ...
    /// <summary>An overload-able point for initialization logic which requires the <see cref="Id" /> and the <see cref="Owner" /> to be set.</summary>
