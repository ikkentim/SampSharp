...
/// <summary>Represents the default commands manager.</summary>
public class CommandsManager : ICommandsManager
{
    ...
    /// <summary>Initializes a new instance of the <see cref="CommandsManager" /> class.</summary>
    ...
    /// <summary>Registers the specified command.</summary>
    ...
    public virtual void Register(CommandPath[] commandPaths, string displayName, bool ignoreCase, IPermissionChecker[] permissionCheckers, MethodInfo method,
        string usageMessage)
    {
        Register(CreateCommand(commandPaths, displayName, ignoreCase, permissionCheckers, method, usageMessage));
    }
    ...
    protected virtual ICommand CreateCommand(CommandPath[] commandPaths, string displayName, bool ignoreCase, IPermissionChecker[] permissionCheckers,
    ...
    /// <summary>Creates a help command.</summary>
    /// <param name="commandPaths">The command paths.</param>
    /// <param name="displayName">The display name.</param>
    /// <param name="ignoreCase">if set to <c>true</c> ignore the case the command.</param>
    /// <param name="permissionCheckers">The permission checkers.</param>
    ...
    /// <param name="usageMessage">The usage message.</param>
    /// <returns>The created command</returns>
    protected virtual ICommand CreateHelpCommand(CommandPath[] commandPaths, string displayName, bool ignoreCase, IPermissionChecker[] permissionCheckers,
        MethodInfo method, string usageMessage)
    {
    ...
    {
        if (type == null || !typeof(IPermissionChecker).GetTypeInfo()
    ...
    private static IEnumerable<IPermissionChecker> GetCommandPermissionCheckers(Type type)
    {
        ...
        foreach (var permissionChecker in type.GetTypeInfo()
    ...
    private static IEnumerable<IPermissionChecker> GetCommandPermissionCheckers(MethodInfo method)
    {
        ...
        foreach (var permissionChecker in method.GetCustomAttributes<CommandGroupAttribute>()
                     .Select(a => a.PermissionChecker)
    ...
    {
        var count = 0;
        var groups = method.GetCustomAttributes<CommandGroupAttribute>()
    ...
    /// <summary>Gets the command for the specified command text.</summary>
    ...
    /// <summary>Gets a read-only collection of all registered commands.</summary>
    public virtual IReadOnlyCollection<ICommand> Commands => _commands.AsReadOnly();

    /// <summary>Loads all tagged commands from the assembly containing the specified type.</summary>
    ...
    public virtual void RegisterCommands(Type typeInAssembly)
    {
        ...
        RegisterCommands(typeInAssembly.GetTypeInfo()
            .Assembly);
    }

    /// <summary>Loads all tagged commands from the specified <paramref name="assembly" />.</summary>
    /// <param name="assembly">The assembly to load the commands from.</param>
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Major Code Smell",
        "S3011:Reflection should not be used to increase accessibility of classes, methods, or fields",
        Justification = "Commands can live in type of any visibility")]
    public virtual void RegisterCommands(Assembly assembly)
    {
        foreach (var method in assembly.GetTypes()
                     ...
                     .Where(type => !type.GetTypeInfo()
                         .IsInterface && type.GetTypeInfo()
                         .IsClass)
                     // Select the methods in the type.
                     .SelectMany(type => type.GetTypeInfo()
                     ...
                     .Where(DefaultCommand.IsValidCommandMethod))
        {
            ...
            {
                var helpCommandPaths = GetCommandGroupPaths(method)
                    .Select(g => new CommandPath(g))
                    .ToArray();

                if (helpCommandPaths.Length > 0)
                {
                    Register(CreateHelpCommand(helpCommandPaths, attribute.DisplayName, attribute.IgnoreCase, GetCommandPermissionCheckers(method)
                        .ToArray(), method, attribute.UsageMessage));
                }
            ...
            var commandPaths = GetCommandGroupPaths(method)
            ...
            Register(commandPaths.ToArray(), attribute.DisplayName, attribute.IgnoreCase, GetCommandPermissionCheckers(method)
                .ToArray(), method, attribute.UsageMessage);
        }
    }
    ...
    /// <summary>Processes the specified player.</summary>
    /// <param name="commandText">The command text.</param>
    /// <param name="player">The player.</param>
