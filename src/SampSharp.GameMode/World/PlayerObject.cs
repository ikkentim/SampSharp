...
/// <summary>Represents a player-object.</summary>
public partial class PlayerObject : IdentifiedOwnedPool<PlayerObject, BasePlayer>, IGameObject
{
    ...
    /// <summary>Maximum number of objects which can exist.</summary>
    public const int Max = 1000;

    /// <summary>Gets the rotation of this <see cref="IGameObject" />.</summary>
    ...
    /// <summary>Gets the position of this <see cref="IWorldObject" />.</summary>
    ...
    /// <summary>Gets whether this <see cref="IGameObject" /> is moving.</summary>
    public virtual bool IsMoving => PlayerObjectInternal.Instance.IsPlayerObjectMoving(Owner.Id, Id);

    /// <summary>Gets whether this <see cref="IGameObject" /> is valid.</summary>
    public virtual bool IsValid => PlayerObjectInternal.Instance.IsValidPlayerObject(Owner.Id, Id);

    /// <summary>Gets the model of this <see cref="IGameObject" />.</summary>
    ...
    /// <summary>Gets the draw distance of this <see cref="IGameObject" />.</summary>
    public virtual float DrawDistance { get; }

    /// <summary>
    /// Occurs when the <see cref="OnMoved" /> callback is being called. This callback is called when an object is moved after
    /// <see cref="Move(Vector3,float)" /> (when it stops moving).
    /// </summary>
    public event EventHandler<EventArgs> Moved;

    /// <summary>
    /// Occurs when the <see cref="OnSelected" /> callback is being called. This callback is called when a player selects an object after
    /// <see cref="Select" /> has been used.
    /// </summary>
    public event EventHandler<SelectPlayerObjectEventArgs> Selected;

    /// <summary>Occurs when the <see cref="OnEdited" /> callback is being called. This callback is called when a player ends object edition mode.</summary>
    public event EventHandler<EditPlayerObjectEventArgs> Edited;

    /// <summary>Initializes a new instance of the <see cref="PlayerObject" /> class.</summary>
    public PlayerObject()
    {
    }
    ...
    public PlayerObject(BasePlayer owner, int modelid, Vector3 position, Vector3 rotation) : this(owner, modelid, position, rotation, 0)
    {
    }
    ...
    public PlayerObject(BasePlayer owner, int modelid, Vector3 position, Vector3 rotation, float drawDistance)
    {
        ...
        Id = PlayerObjectInternal.Instance.CreatePlayerObject(owner.Id, modelid, position.X, position.Y, position.Z, rotation.X, rotation.Y, rotation.Z,
            drawDistance);
    }

    /// <summary>Moves this <see cref="IGameObject" /> to the given position and rotation with the given speed.</summary>
    ...
    public virtual int Move(Vector3 position, float speed, Vector3 rotation)
    {
        ...
        return PlayerObjectInternal.Instance.MovePlayerObject(Owner.Id, Id, position.X, position.Y, position.Z, speed, rotation.X, rotation.Y, rotation.Z);
    }

    /// <summary>Moves this <see cref="IGameObject" /> to the given position with the given speed.</summary>
    /// <param name="position">The position to which to move this <see cref="IGameObject" />.</param>
    /// <param name="speed">The speed at which to move this <see cref="IGameObject" />.</param>
    /// <returns>The time it will take for the object to move in milliseconds.</returns>
    public virtual int Move(Vector3 position, float speed)
    {
        ...
        return PlayerObjectInternal.Instance.MovePlayerObject(Owner.Id, Id, position.X, position.Y, position.Z, speed, -1000, -1000, -1000);
    }

    /// <summary>Stop this <see cref="IGameObject" /> from moving any further.</summary>
    ...
    /// <summary>Sets the material of this <see cref="IGameObject" />.</summary>
    /// <param name="materialindex">The material index on the object to change.</param>
    /// <param name="modelid">
    /// The model identifier on which the replacement texture is located. Use 0 for alpha. Use -1 to change the material color without altering
    /// the texture.
    /// </param>
    ...
    /// <summary>Sets the material text of this <see cref="IGameObject" />.</summary>
    /// <param name="materialindex">The material index on the object to change.</param>
    ...
    public virtual void SetMaterialText(int materialindex, string text, ObjectMaterialSize materialsize, string fontface, int fontsize, bool bold,
        Color foreColor, Color backColor, ObjectMaterialTextAlign textalignment)
    {
        AssertNotDisposed();

        PlayerObjectInternal.Instance.SetPlayerObjectMaterialText(Owner.Id, Id, text, materialindex, (int)materialsize, fontface, fontsize, bold,
            foreColor.ToInteger(ColorFormat.ARGB), backColor.ToInteger(ColorFormat.ARGB), (int)textalignment);
    }

    /// <summary>Disable collisions between players' cameras and this <see cref="PlayerObject" />.</summary>
    ...
    /// <summary>Attaches this <see cref="PlayerObject" /> to the specified <paramref name="player" />.</summary>
    ...
    public virtual void AttachTo(BasePlayer player, Vector3 offset, Vector3 rotation)
    {
        ...
        PlayerObjectInternal.Instance.AttachPlayerObjectToPlayer(Owner.Id, Id, player.Id, offset.X, offset.Y, offset.Z, rotation.X, rotation.Y, rotation.Z);
    }

    /// <summary>Attaches this <see cref="PlayerObject" /> to the specified <paramref name="vehicle" />.</summary>
    ...
    public virtual void AttachTo(BaseVehicle vehicle, Vector3 offset, Vector3 rotation)
    {
        ...
        PlayerObjectInternal.Instance.AttachPlayerObjectToVehicle(Owner.Id, Id, vehicle.Id, offset.X, offset.Y, offset.Z, rotation.X, rotation.Y, rotation.Z);
    }

    /// <summary>Attaches the player's camera to this <see cref="PlayerObject" />.</summary>
    /// <remarks>This will attach the camera of the player whose object this is to this object.</remarks>
    ...
    /// <summary>Lets the <see cref="IdentifiedOwnedPool{TInstance,TOwner}.Owner" /> of this <see cref="PlayerObject" /> edit this object.</summary>
    ...
    /// <summary>Lets the specified <paramref name="player" /> select an object.</summary>
    /// <param name="player">The player.</param>
    /// <exception cref="System.ArgumentNullException">player</exception>
    ...
    /// <summary>Raises the <see cref="Moved" /> event.</summary>
