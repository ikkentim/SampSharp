...
/// <summary>Represents a list of <see cref="IController" /> instances.</summary>
public class ControllerCollection : Disposable, IEnumerable<IController>
{
    private readonly List<IController> _controllers = new();

    /// <summary>Returns an enumerator that iterates through this collection.</summary>
    /// <returns>A <see cref="T:System.Collections.Generic.List`1.Enumerator" /> for this collection.</returns>
    ...
    /// <summary>Adds a <see cref="IController" /> to this collection.</summary>
    ...
    /// <summary>Adds a <see cref="IController" /> to this collection and remove controllers it overrides.</summary>
    /// <param name="controller">The <see cref="IController" /> to add to this collection.</param>
    public void Override(IController controller)
    {
        var overrides = this.Where(c => c.GetType()
                .GetTypeInfo()
                .IsInstanceOfType(controller))
            .ToArray();

        if (overrides.Any())
            CoreLog.Log(CoreLogLevel.Debug, $"{controller} overrides {string.Join(", ", overrides.Select(x => (object)x))}");
    ...
    /// <summary>Removes a <see cref="IController" /> from this collection.</summary>
    ...
    /// <summary>Removes all <see cref="IController" /> instances of the given type from this collection.</summary>
    ...
    /// <summary>Get the first instance of <see cref="IController" /> of the given type.</summary>
