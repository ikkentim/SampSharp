...
public class GlobalObject : Component
{
    ...
    public virtual Vector3 Rotation
    {
        get
        {
            GetComponent<NativeObject>()
                .GetObjectRot(out var x, out var y, out var z);
            return new Vector3(x, y, z);
        }
        set => GetComponent<NativeObject>()
            .SetObjectRot(value.X, value.Y, value.Z);
    }
    ...
    public virtual Vector3 Position
    {
        ...
        {
            GetComponent<NativeObject>()
                .GetObjectPos(out var x, out var y, out var z);
            return new Vector3(x, y, z);
        }
        set => GetComponent<NativeObject>()
            .SetObjectPos(value.X, value.Y, value.Z);
    }
    ...
    {
        GetComponent<NativeObject>()
            .SetObjectMaterialText(text, materialIndex, (int)materialSize, fontface, fontSize, bold, foreColor.ToInteger(ColorFormat.ARGB),
    ...
    /// <summary>Attaches this object to the specified player, vehicle or object.</summary>
    ...
    public virtual void AttachTo(EntityId target, Vector3 offset, Vector3 rotation, bool syncRotation = false)
    {
        ...
        if (target.IsOfType(SampEntities.PlayerType))
            GetComponent<NativeObject>()
                .AttachObjectToPlayer(target, offset.X, offset.Y, offset.Z, rotation.X, rotation.Y, rotation.Z);
        else if (target.IsOfType(SampEntities.VehicleType))
            GetComponent<NativeObject>()
                .AttachObjectToVehicle(target, offset.X, offset.Y, offset.Z, rotation.X, rotation.Y, rotation.Z);
        else if (target.IsOfType(SampEntities.ObjectType))
            GetComponent<NativeObject>()
                .AttachObjectToObject(target, offset.X, offset.Y, offset.Z, rotation.X, rotation.Y, rotation.Z, syncRotation);
        else
