...
/// <summary>A parser for an <see cref="float" /> parameter.</summary>
public class FloatParser : ICommandParameterParser
{
    ...
    {
        if (!_wordParser.TryParse(services, ref inputText, out var subResult) || !(subResult is string word) || !float.TryParse(word, out var num))
        {
            result = null;
            return false;
        }
