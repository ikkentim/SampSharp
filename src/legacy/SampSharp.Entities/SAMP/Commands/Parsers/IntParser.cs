...
/// <summary>A parser for an <see cref="int" /> parameter.</summary>
public class IntParser : ICommandParameterParser
{
    ...
    {
        if (!_wordParser.TryParse(services, ref inputText, out var subResult) || !(subResult is string word) || !int.TryParse(word, out var num))
        {
            result = null;
            return false;
        }
