...
/// <summary>Base class for a SA-MP game mode.</summary>
public abstract partial class BaseMode : Disposable, IGameModeProvider
{
    ...
    /// <summary>Initializes a new instance of the <see cref="BaseMode" /> class.</summary>
    protected BaseMode()
    {
        Instance = this;
    }

    /// <summary>Gets the collection of controllers loaded.</summary>
    protected virtual ControllerCollection Controllers => _controllers;

    /// <summary>Gets the <see cref="GameModeServiceContainer" /> holding all the service providers attached to the game mode.</summary>
    public virtual GameModeServiceContainer Services { get; } = new();

    /// <summary>Gets the game mode client.</summary>
    public IGameModeClient Client { get; private set; }

    /// <summary>Auto-loads the controllers in the specified assembly.</summary>
    /// <param name="assembly">The assembly.</param>
    public void AutoloadControllersForAssembly(Assembly assembly)
    {
        foreach (var type in assembly.GetExportedTypes()
                     .Where(t => t.GetTypeInfo()
                         .IsClass && typeof(IController).GetTypeInfo()
                         .IsAssignableFrom(t) && t.GetTypeInfo()
                         .GetCustomAttribute<ControllerAttribute>() != null))
        {
    ...
    /// <summary>Loads all controllers into the given ControllerCollection.</summary>
    ...
    {
        AutoloadControllersForAssembly(typeof(BaseMode).GetTypeInfo()
            .Assembly);
        AutoloadControllersForAssembly(GetType()
            .GetTypeInfo()
            .Assembly);
    }
    ...
    private void AutoloadPoolTypes()
    {
        ...
        foreach (var poolType in new[] { typeof(BaseMode), GetType() }.Concat(_extensions.Select(e => e.GetType()))
                     ...
                     .Where(t => t.GetTypeInfo()
                         .IsClass && t.GetTypeInfo()
                         .GetCustomAttribute<PooledTypeAttribute>() != null)
                     .Distinct())
        {
            // If poolType or subclass of poolType is already in types, continue.
            if (types.Any(t => t == poolType || poolType.GetTypeInfo()
                    .IsAssignableFrom(t)))
            {
                CoreLog.Log(CoreLogLevel.Debug, $"Pool of type {poolType} is not auto-loaded because a subclass of it will already be loaded.");
                continue;
            }

            // Remove all types in types where type is parent type of poolType.
            foreach (var t in types.Where(t => t.GetTypeInfo()
                             .IsAssignableFrom(poolType))
                         .ToArray())
            {
                CoreLog.Log(CoreLogLevel.Debug, $"No longer auto-loading type {poolType} because a subclass of it is going to be loaded.");
                types.Remove(t);
            }
        ...
        var poolTypes = new[] { typeof(IdentifiedPool<>), typeof(IdentifiedOwnedPool<,>) };
        foreach (var type in types)
        {
            var pool = type;
            do
            {
                pool = pool.GetTypeInfo()
                    .BaseType;
            } while (pool != null && (!pool.GetTypeInfo()
                         .IsGenericType || !poolTypes.Contains(pool.GetGenericTypeDefinition())));
            ...
            pool.GetTypeInfo()
                .GetMethod("Register", new[] { typeof(Type) })!.Invoke(null, new object[] { type });
        }
    }
    ...
    private static void AddExtensionToLoadList(Assembly assembly, List<Assembly> load, List<Assembly> loading)
    {
        ...
        if (!assembly.GetCustomAttributes<SampSharpExtensionAttribute>()
                .Any())
        ...
        foreach (var dependency in assembly.GetCustomAttributes<SampSharpExtensionAttribute>()
    ...
    private void LoadExtensions()
    {
        ...
        foreach (var assembly in GetType()
                     .GetTypeInfo()
                     ...
                     .Distinct()
                     .Where(a => a.GetCustomAttributes<SampSharpExtensionAttribute>()
                         .Any()))
        ...
        foreach (var assembly in load)
        {
            ...
            foreach (var extensionType in attributes.Select(attribute => attribute.Type))
            {
                if (extensionType == null)
                    continue;
                if (!typeof(IExtension).GetTypeInfo()
                        .IsAssignableFrom(extensionType))
                {
                    CoreLog.Log(CoreLogLevel.Warning,
                        $"The extension from {assembly} could not be loaded. The specified extension type does not inherit from IExtension.");
                    continue;
                }

                if (!extensionType.GetTypeInfo()
                        .Assembly.Equals(assembly))
                {
                    CoreLog.Log(CoreLogLevel.Warning,
                        $"The extension from {assembly} could not be loaded. The specified extension type is not part of the assembly.");
                    continue;
                }
    ...
    /// <summary>Registers the specified extension.</summary>
