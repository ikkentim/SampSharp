...
public abstract partial class BaseMode
{
    ...
    /// <summary>Disables the name tag line of sight test.</summary>
    ...
    /// <summary>
    /// A function that can be used in <see cref="OnInitialized" /> to enable or disable the players markers, which would normally be shown on the radar. If
    /// you want to change the marker settings at some other point in the game mode, have a look at <see cref="BasePlayer.SetPlayerMarker" /> which does exactly that.
    /// </summary>
    ...
    /// <summary>Toggle the drawing of player name tags, health bars and armor bars above players.</summary>
    ...
    /// <summary>Uses standard player walking animation (animation of CJ) instead of custom animations for every skin (e.g. skating for skater skins).</summary>
    public virtual void UsePlayerPedAnimations()
    {
        BaseModeInternal.Instance.UsePlayerPedAnims();
    }

    /// <summary>Enable friendly fire for team vehicles.</summary>
    /// <remarks>Players will be unable to damage teammates' vehicles (<see cref="BasePlayer.Team" /> must be used!)</remarks>
    ...
    /// teams used for better (more effective) internal handling. This function should only be used in the <see cref="OnInitialized" /> callback.
    /// </summary>
    ...
    public virtual int AddPlayerClass(int modelid, Vector3 position, float zAngle, Weapon weapon1, int weapon1Ammo, Weapon weapon2, int weapon2Ammo,
        Weapon weapon3, int weapon3Ammo)
    {
        return BaseModeInternal.Instance.AddPlayerClass(modelid, position.X, position.Y, position.Z, zAngle, (int)weapon1, weapon1Ammo, (int)weapon2,
    ...
    public virtual int AddPlayerClass(int teamid, int modelid, Vector3 position, float zAngle, Weapon weapon1, int weapon1Ammo, Weapon weapon2, int weapon2Ammo,
        Weapon weapon3, int weapon3Ammo)
    {
        return BaseModeInternal.Instance.AddPlayerClassEx(teamid, modelid, position.X, position.Y, position.Z, zAngle, (int)weapon1, weapon1Ammo, (int)weapon2,
            weapon2Ammo, (int)weapon3, weapon3Ammo);
    }
    ...
    /// <param name="weaponAmmo">The amount of ammunition for the spawn weapon.</param>
    /// <returns>The ID of the class that was just created.</returns>
    public virtual int AddPlayerClass(int teamid, int modelid, Vector3 position, float zAngle, Weapon weapon, int weaponAmmo)
    {
    ...
    /// <summary>Enables or disables stunt bonuses for all players.</summary>
    ...
    /// <summary>
    /// Use this function before any player connects (<see cref="OnInitialized" />) to tell all clients that the script will control vehicle engines and
    ...
    /// <summary>Ends and restarts the game mode.</summary>
    ...
    /// <summary>With this function you can enable or disable tire popping.</summary>
