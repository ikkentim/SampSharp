...
public class SystemDialogsTest : ISystem
{
    ...
    {
        dialogService.Show(player.Entity, new TablistDialog("Hello", "Left", "right", "Column1", "Column2", "Column3")
        {
            { "r1c1", "r1c2", "r1c3" },
        ...
        {
            player.SendClientMessage($"Resp: {r.Response} {r.ItemIndex}: ({string.Join(" ", r.Item?.Columns!)},{r.Item?.Tag})");
        });
    ...
    {
        dialogService.Show(player.Entity, new MessageDialog("Hello", "Hello, world! " + DateTime.Now, "Left", "right"),
